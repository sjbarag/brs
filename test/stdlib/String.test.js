...
describe("global string functions", () => {
    describe("UCase", () => {
        it("converts a BRS string to its uppercase form", async () => {
            expect(
                await UCase.call(interpreter, new BrsString("l0rEm"))
    ...
    describe("LCase", () => {
        it("converts a BRS string to its lowercase form", async () => {
            expect(
                await LCase.call(interpreter, new BrsString("l0rEm"))
            ).toEqual(new BrsString("l0rem"));
        });
    });
    ...
    describe("Asc", () => {
        it("converts an empty BRS string to zero", async () => {
            expect(
                await Asc.call(interpreter, new BrsString(""))
            ).toEqual(new Int32(0));
        });

        it("converts a character to a UTF-16 representation", async () => {
            expect(
                await Asc.call(interpreter, new BrsString("for"))
            ).toEqual(new Int32(102)); // UTF-16 decimal for "f"

            expect(
                await Asc.call(interpreter, new BrsString("ぇ"))
    ...
    describe("Chr", () => {
        it("converts a negative or zero to an empty string", async () => {
            expect(
                await Chr.call(interpreter, new Int32(-1))
            ).toEqual(new BrsString(""));

            expect(
                await Chr.call(interpreter, new Int32(0))
            ).toEqual(new BrsString(""));
        });

        it("converts an UTF-16 integer to character", async () => {
            expect(
                await Chr.call(interpreter, new Int32(34))
            ).toEqual(new BrsString("\""));

            expect(
                await Chr.call(interpreter, new Int32(12359))
    ...
    describe("Left", () => {
        it("get first n characters in a string longer than n characters", async () => {
            expect(
                await Left.call(interpreter, new BrsString("pineapple"), new Int32(4))
            ).toEqual(new BrsString("pine"));
        });

        it("get the original string back with n larger than string length", async () => {
            expect(
                await Left.call(interpreter, new BrsString("boy"), new Int32(5))
            ).toEqual(new BrsString("boy"));
        });

        it("get back empty string when character length is 0 or negative", async () => {
            expect(
                await Left.call(interpreter, new BrsString("apple"), new Int32(0))
            ).toEqual(new BrsString(""));

            expect(
                await Left.call(interpreter, new BrsString("apple"), new Int32(-5))
            ).toEqual(new BrsString(""));
        });
    });
    ...
    describe("Right", () => {
        it("returns the last (n) characters from a string", async () => {
            expect(
                await Right.call(interpreter, new BrsString("pineapple"), new Int32(4))
            ).toEqual(new BrsString("pple"));
        });

        it("returns original string when (n) is longer than length", async () => {
            expect(
                await Right.call(interpreter, new BrsString("boy"), new Int32(5))
            ).toEqual(new BrsString("boy"));
        });

        it("returns an empty string", async () => {
            expect(
                await Right.call(interpreter, new BrsString("apple"), new Int32(0))
            ).toEqual(new BrsString(""));

            expect(
                await Right.call(interpreter, new BrsString("apple"), new Int32(-5))
            ).toEqual(new BrsString(""));
        });
    });
    ...
    describe("Instr", () => {
        it("returns 0 if the string is not found", async () => {
            expect(
                await Instr.call(interpreter, new Int32(1), new BrsString("apple"), new BrsString("orange"))
            ).toEqual(new Int32(0));
        });

        it("returns the index of the first found string", async () => {
            expect(
                await Instr.call(interpreter, new Int32(1), new BrsString("apple"), new BrsString("a"))
            ).toEqual(new Int32(1));
        });

        it("returns the index of the first found after the starting index passed in", async () => {
            expect(
                await Instr.call(interpreter, new Int32(3), new BrsString("apple"), new BrsString("p"))
            ).toEqual(new Int32(3));
        });
    });
    ...
    describe("Len", () => {
       it("returns the length of the passed in string", async () => {
           expect(
               await Len.call(interpreter, new BrsString("abc"))
           ).toEqual(new Int32(3));
       });

       it("returns zero with an empty string", async () => {
            expect(
                await Len.call(interpreter, new BrsString(""))
            ).toEqual(new Int32(0));
       });
    });
    ...
    describe("Mid", () => {
        ...
        it("return the end of the string with only the position specified", async () => {
            expect(
                await Mid.call(interpreter, new BrsString("abc"), new Int32(2))
    ...
    describe("StrI", () => {
        it("returns a string from a positive integer", async () => {
            expect(
                await StrI.call(interpreter, new Int32(3))
        ...
        it("returns a string from a zero integer", async () => {
            expect(
                await StrI.call(interpreter, new Int32(0))
            ).toEqual(new BrsString("0"));
        });

        it("returns an empty string for invalid radices", async () => {
            expect(
                await StrI.call(interpreter, new Int32(0), new Int32(1))
            ).toEqual(new BrsString(""));

            expect(
                await StrI.call(interpreter, new Int32(0), new Int32(37))
            ).toEqual(new BrsString(""));
        })

        it("returns a string formatted by a radix", async () => {
            expect(
                await StrI.call(interpreter, new Int32(255), new Int32(16))
    ...
    describe("Val", () => {
        it("returns a float from a positive decimal-formatted string", async () => {
            expect(
                await Val.call(interpreter, new BrsString("12.34"))
            ).toEqual(new Float(12.34));
        });

        it("returns a float from a negative decimal-formatted string", async () => {
            expect(
                await Val.call(interpreter, new BrsString("-32.34"))
            ).toEqual(new Float(-32.34));
        });

        it("returns a float from a zero decimal-formatted string", async () => {
            expect(
                await Val.call(interpreter, new BrsString("0.0"))
            ).toEqual(new Float(0.0));
        })

        it("returns an integer from a string an radix", async () => {
            expect(
                await Val.call(interpreter, new BrsString("7"), new Int32(10))
            ).toEqual(new Int32(7));

            expect(
                await Val.call(interpreter, new BrsString("0x80"), new Int32(0))
            ).toEqual(new Int32(128));

            expect(
                await Val.call(interpreter, new BrsString("FF"), new Int32(16))
            ).toEqual(new Int32(255));

            expect(
                await Val.call(interpreter, new BrsString("1001"), new Int32(2))
            ).toEqual(new Int32(9));
        })
    });
